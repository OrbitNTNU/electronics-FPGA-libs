...
module Slave_modul #(parameter frame_size)
 (
    ...
    reg[4:0] recieve_bit_counter = 0; //counter for which bit is recieved
    reg[5:0] send_bit_counter = 0; // counter for which bit is sendt
...
always_comb begin
    msg_r_l = msg_recieved && !Module_ready;
end
